...
<el-scrollbar>
  <div>
        ...
        <el-form-item :label="$t('fm.config.widget.widgetType')">
          <el-tag effect="plain">
            ...
            <template v-if="data.type === 'custom'">
              <i class="icon fm-iconfont icon-extend" style="font-size: 14px;"></i>&nbsp;
              {{$t(`fm.components.custom.title`)}}
            </template>
            <template v-else>
              <i :class="`fm-iconfont ${data.icon}`" style="font-size: 14px;"></i>&nbsp;
              {{$t(`fm.components.fields.${data.type}`)}}
            </template>
          </el-tag>
        </el-form-item>
        <el-collapse class="fm-widget-config-collapse" :model-value="['basic', 'options', 'expend', 'columns', 'operation', 'paging']">
          <el-collapse-item :title="$t('fm.config.widget.basicSetting')" name="basic">
            <el-form-item v-if="data.type != 'td' && data.type != 'th' && data.type != 'col'" :required="true" prop="model" for="">
              <template #label>
                {{$t('fm.config.widget.model')}} 
                <i class="fm-iconfont icon-copy" :title="$t('fm.tooltip.copy')" @click="handleCopyField" style="cursor: pointer;font-size: 18px;padding:0 5px;color:var(--fm-color-primary);"></i> 
              </template>
              <div v-if="getModelNode()" style="font-size: 13px; word-break: break-all; line-height: 1.2; color: #e6a23c; padding: 0 5px 5px 5px;">
                {{getModelNode()}}
              </div>
              <div style="display: flex;" class="clipboard-content">
            ...
            <el-form-item :label="$t('fm.config.widget.width')" v-if="Object.keys(data.options).indexOf('width')>=0 && data.type != 'td'">
              <el-input clearable v-model="data.options.width" :disabled="getVantMobile()"></el-input>
            </el-form-item>
            ...
            <el-form-item :label="$t('fm.config.widget.tip')" v-if="Object.keys(data.options).indexOf('tip')>=0">
              <el-input type="textarea" autosize clearable v-model="data.options.tip" :disabled="getVantMobile()"></el-input>
            </el-form-item>
            ...
            <el-form-item :label="$t('fm.config.widget.effect')" v-if="Object.keys(data.options).indexOf('effect')>=0">
              <el-radio-group v-model="data.options.effect" :disabled="getVantMobile()">
            ...
            <el-form-item :label="$t('fm.config.widget.showInput')" v-if="Object.keys(data.options).indexOf('showInput')>=0">
              <el-switch v-model="data.options.showInput" :disabled="getVantMobile()"></el-switch>
            </el-form-item>
            ...
            <el-form-item :label="$t('fm.config.widget.controls')" v-if="Object.keys(data.options).indexOf('controls')>=0">
              <el-switch v-model="data.options.controls" :disabled="getVantMobile()"></el-switch>
            </el-form-item>
            ...
            <el-form-item :label="$t('fm.config.widget.controlsPosition')" v-if="Object.keys(data.options).indexOf('controlsPosition') >= 0 && data.options.controls">
              <el-radio-group v-model="data.options.controlsPosition" :disabled="getVantMobile()">
            ...
            ">
              <el-form-item :label="$t('fm.config.widget.flex')">
                <el-switch v-model="data.options.flex" :disabled="getVantMobile()"></el-switch>
              </el-form-item>
              <template v-if="data.type == 'inline'">
                <el-form-item :label="$t('fm.config.widget.justify')" v-if="data.options.flex">
                  <el-radio-group v-model="data.options.flexJustify" size="small" :disabled="getVantMobile()">
                    <el-radio-button value="flex-start">
                      <i class="fm-iconfont icon-justify-content-flex-start-icon" ></i>
                    </el-radio-button>
                    <el-radio-button value="flex-end">
                      <i class="fm-iconfont icon-justify-content-flex-end-icon"></i>
                    </el-radio-button>
                    <el-radio-button value="center">
                      <i class="fm-iconfont icon-justify-content-center-icon"></i>
                    </el-radio-button>
                    <el-radio-button value="space-between">
                      <i class="fm-iconfont icon-justify-content-space-between-icon"></i>
                    </el-radio-button>
                    <el-radio-button value="space-around">
                      <i class="fm-iconfont icon-justify-content-space-around-icon"></i>
                    </el-radio-button>
                    <el-radio-button value="space-evenly">
                      <i class="fm-iconfont icon-justify-content-space-evenly-icon"></i>
                    </el-radio-button>
                  </el-radio-group>
                </el-form-item>
                <el-form-item :label="$t('fm.config.widget.align')" v-if="data.options.flex">
                  <el-radio-group v-model="data.options.flexAlign" size="small" :disabled="getVantMobile()">
                    <el-radio-button value="flex-start">
                      <i class="fm-iconfont icon-align-items-flex-start" ></i>
                    </el-radio-button>
                    <el-radio-button value="flex-end">
                      <i class="fm-iconfont icon-align-items-flex-end"></i>
                    </el-radio-button>
                    <el-radio-button value="center">
                      <i class="fm-iconfont icon-align-items-center"></i>
                    </el-radio-button>
                    <el-radio-button value="stretch">
                      <i class="fm-iconfont icon-align-items-stretch"></i>
                    </el-radio-button>
                  </el-radio-group>
                </el-form-item>
              </template>
            ...
            <!-- td -->
            <template v-if="data.type == 'td'">
              <el-form-item :label="$t('fm.config.widget.textAlign')" >
                <el-radio-group v-model="data.options.textAlign" size="small">
                  <el-radio-button value="left">
                    <i class="fm-iconfont icon-text-align-left" ></i>
                  </el-radio-button>
                  <el-radio-button value="center">
                    <i class="fm-iconfont icon-text-align-center"></i>
                  </el-radio-button>
                  <el-radio-button value="right">
                    <i class="fm-iconfont icon-text-align-right"></i>
                  </el-radio-button>
                </el-radio-group>
              </el-form-item>
              <el-form-item :label="$t('fm.config.widget.verticalAlign')">
                <el-radio-group v-model="data.options.verticalAlign" size="small">
                  <el-radio-button value="top">
                    <i class="fm-iconfont icon-ic24-vertical-align-top" ></i>
                  </el-radio-button>
                  <el-radio-button value="middle">
                    <i class="fm-iconfont icon-vertical-align-middle-outlined"></i>
                  </el-radio-button>
                  <el-radio-button value="bottom">
                    <i class="fm-iconfont icon-ic24-vertical-align-bottom"></i>
                  </el-radio-button>
                </el-radio-group>
              </el-form-item>
            </template>
            ...
            <template v-if="Object.keys(data.options).indexOf('steps')>=0">
              ...
              <el-form-item :label="$t('fm.config.widget.space')">
                <el-input-number v-model="data.options.space" :min="0" :step="10" :disabled="getVantMobile()"></el-input-number>
              </el-form-item>
              ...
              <el-form-item :label="$t('fm.config.widget.processStatus')">
                <el-select v-model="data.options.processStatus" :disabled="getVantMobile()">
              ...
              <el-form-item :label="$t('fm.config.widget.finishStatus')">
                <el-select v-model="data.options.finishStatus" :disabled="getVantMobile()">
                  <el-option value="wait" label="wait"></el-option>
                  <el-option value="process" label="process"></el-option>
                  <el-option value="finish" label="finish"></el-option>
                  <el-option value="error" label="error"></el-option>
            ...
            <template v-if="data.type == 'alert'">
              ...
              <el-form-item :label="$t('fm.config.widget.center')">
                <el-switch v-model="data.options.center" :disabled="getVantMobile()"></el-switch>
              </el-form-item>
            ...
            <el-form-item :label="$t('fm.config.widget.selection')" v-if="data.type == 'table'">
            ...
            <template v-if="data.type == 'time' || data.type == 'date'">
              ...
              <el-form-item :label="$t('fm.config.widget.startPlaceholder')" v-if="(data.options.isRange) || data.options.type=='datetimerange' || data.options.type=='daterange'">
                <el-input clearable v-model="data.options.startPlaceholder" :disabled="getVantMobile()"></el-input>
              </el-form-item>
              <el-form-item :label="$t('fm.config.widget.endPlaceholder')" v-if="data.options.isRange || data.options.type=='datetimerange' || data.options.type=='daterange'">
                <el-input clearable v-model="data.options.endPlaceholder" :disabled="getVantMobile()"></el-input>
              </el-form-item>
              ...
              <el-form-item :label="$t('fm.config.widget.defaultValue')" v-if="data.type=='date'">
                <el-date-picker
                  ...
                  :clearable="true"
                  :value-format="data.options.timestamp ? 'x' : data.options.format"
            ...
            <template v-if="data.type=='imgupload' || data.type=='fileupload'">
              <el-form-item :label="$t('fm.config.widget.accept')" v-if="data.type=='fileupload'">
                <el-input clearable v-model="data.options.accept" :placeholder="$t('fm.config.widget.acceptPlaceholder')"></el-input>
              </el-form-item>
              ...
              <template v-if="data.options.isQiniu">
                ...
                <el-form-item :label="$t('fm.config.widget.tokenFunc')" :required="true" >
                  ...
                  <el-radio-group v-model="data.options.tokenType">
                    ...
                    <el-radio label="fx" value="fx">{{$t('fm.formula.input.fx')}}</el-radio>
                  </el-radio-group>
                  <formula-input size="default" v-if="data.options.tokenType == 'fx'" only v-model="data.options.tokenFx" style="margin-bottom: 5px;"></formula-input>
                  <div v-if="data.options.tokenType == 'datasource'" style="display: flex; align-items: center; margin-bottom: 5px;">
                    <el-select
                    ...
                    <el-tooltip :content="$t('fm.config.widget.settings')">
                      <el-button link type="primary" style="margin: 5px;" @click="handleDatasourceManage(data.options.tokenDataSource)">
                        <i class="fm-iconfont icon-pm"></i>
                      </el-button>
                    </el-tooltip>
                  </div>
                  
                  <object-dynamic :key="data.options.tokenDataSource" :operable="false" v-if="data.options.tokenType == 'datasource' && data.options.remoteArgs" v-model="data.options.remoteArgs"></object-dynamic>
              ...
              <template v-else>
                ...
                <el-form-item :label="$t('fm.config.widget.headers')">
                  <array-dynamic v-model="data.options.headers"></array-dynamic>
                </el-form-item>
            ...
            <template v-if="data.type == 'inline'">
              <el-form-item :label="$t('fm.config.widget.spaceSize')" >
                <el-input-number clearable :min="0" v-model="data.options.spaceSize" :disabled="getVantMobile()"></el-input-number>
              </el-form-item>
            </template>
            ...
            <template v-if="data.type == 'tabs'">
              ...
              <el-form-item :label="$t('fm.config.widget.tabPosition')">
                <el-radio-group v-model="data.options.tabPosition" :disabled="getVantMobile()">
            ...
            <template v-if="data.type == 'card'">
              <el-form-item :label="$t('fm.config.widget.padding')">
                <el-input v-model="data.options.padding" clearable :disabled="getVantMobile()"></el-input>
              </el-form-item>
              ...
              <el-form-item :label="$t('fm.config.widget.shadow')">
                <el-radio-group v-model="data.options.shadow" :disabled="getVantMobile()">
            ...
            <template v-if="data.type == 'list'">
              <el-form-item :label="$t('fm.config.widget.stripe')">
                <el-switch v-model="data.options.stripe"></el-switch>
              </el-form-item>
              <el-form-item :label="$t('fm.config.widget.bordered')">
                <el-switch v-model="data.options.border"></el-switch>
              </el-form-item>
              <el-form-item :label="$t('fm.config.widget.highlight')">
                <el-switch v-model="data.options.highlight"></el-switch>
              </el-form-item>
              <el-form-item :label="$t('fm.config.widget.selection')">
                <el-switch v-model="data.options.selection"></el-switch>
              </el-form-item>
              <el-form-item :label="$t('fm.config.widget.maxHeight')">
                <el-input clearable v-model.number="data.options.maxHeight">
            ...
            <el-form-item :label="$t('fm.config.widget.defaultValue')" 
              ...
              <el-input-number v-if="data.type=='number' || data.type == 'slider'"
                v-model="data.options.defaultValue"
              ...
              <template v-if="data.type == 'html'">
                <el-popover placement="right" :width="500" trigger="click" :hide-after="0">
                  <template #reference>
                    <el-button style="width: 100%;">{{$t('fm.config.widget.setting')}}</el-button>
                  </template>
                  <code-editor :key="data.key" v-model="data.options.defaultValue" height="300px"></code-editor>
                </el-popover>
              </template>
              ...
              <template v-if="data.type == 'table' || data.type == 'subform' || data.type == 'list'">
                <el-button style="width: 100%;" @click="handleSetDefaultValue">{{$t('fm.config.widget.setting')}}</el-button>
              </template>
            </el-form-item>
          </el-collapse-item>
          <el-collapse-item :title="$t('fm.config.widget.optionsSetting')" name="options"
            v-if="Object.keys(data.options).indexOf('options')>=0 
              || Object.keys(data.options).indexOf('steps')>=0
              || data.type == 'transfer'"
          >
            <el-form-item :label="$t('fm.config.widget.option')" v-if="Object.keys(data.options).indexOf('options')>=0">
              ...
              <template v-if="data.options.remote">
                <div>
                  ...
                  <formula-input size="default" v-if="data.options.remoteType == 'fx'" only v-model="data.options.remoteFx" style="margin-bottom: 5px;"></formula-input>
                  <div v-if="data.options.remoteType == 'datasource'" style="display: flex; align-items: center; margin-bottom: 5px;">
                    <el-select
                      @change="handleDataSourceChange"
                    ...
                    <el-tooltip :content="$t('fm.config.widget.settings')">
                      <el-button link type="primary" style="margin: 5px;" @click="handleDatasourceManage(data.options.remoteDataSource)">
                        <i class="fm-iconfont icon-pm"></i>
                      </el-button>
                    </el-tooltip>
                  </div>
                  <object-dynamic :key="data.options.remoteDataSource" :operable="false" v-if="data.options.remoteType == 'datasource' && data.options.remoteArgs" v-model="data.options.remoteArgs"></object-dynamic>
                  <el-input class="fm-options-item" clearable  v-model="data.options.props.value">
                    <template #prepend>{{$t('fm.config.widget.value')}}</template>
                  </el-input>
                  <el-input class="fm-options-item" clearable  v-model="data.options.props.label">
                    <template #prepend>{{$t('fm.config.widget.label')}}</template>
                  </el-input>
                  <el-input class="fm-options-item" clearable v-model="data.options.props.children">
            ...
            <template v-if="Object.keys(data.options).indexOf('steps')>=0">
              <el-form-item :label="$t('fm.config.widget.steps')">
                ...
                <template v-if="data.options.remote">
                  ...
                  <object-dynamic :key="data.options.remoteDataSource" :operable="false" v-if="data.options.remoteType == 'datasource' && data.options.remoteArgs" v-model="data.options.remoteArgs"></object-dynamic>
                  <el-input class="fm-options-item" clearable  v-model="data.options.props.title">
                    <template #prepend>Title</template>
                    
                  </el-input>
                  <el-input class="fm-options-item" clearable v-model="data.options.props.description">
            ...
            <template v-if="data.type == 'transfer'">
              <el-form-item :label="$t('fm.config.widget.option')">
                ...
                <template v-if="data.options.remote">
                  ...
                  <object-dynamic :key="data.options.remoteDataSource" :operable="false" v-if="data.options.remoteType == 'datasource' && data.options.remoteArgs" v-model="data.options.remoteArgs"></object-dynamic>
                  <el-input class="fm-options-item" clearable  v-model="data.options.props.key">
                    <template #prepend>Key</template>
                  </el-input>
                  <el-input class="fm-options-item" clearable v-model="data.options.props.label">
                    <template #prepend>Label</template>
                  </el-input>
                  <el-input class="fm-options-item" clearable v-model="data.options.props.disabled">
                    <template #prepend>Disabled</template>
                  </el-input>
                </template>
                <template v-else>
                  <el-checkbox-group v-model="data.options.defaultValue">
          ...
          <el-collapse-item :title="$t('fm.config.widget.columnSetting')" name="columns"
            v-if="data.type == 'list'"
          >
            <el-form-item>
              <columns-config v-model="data.options.columns"></columns-config>
            </el-form-item>
            
          </el-collapse-item>
          <el-collapse-item :title="$t('fm.config.widget.operationSetting')" name="operation"
            v-if="data.type == 'list'"
          >
            <el-form-item :label="$t('fm.config.widget.showOperation')">
              <el-switch v-model="data.options.showOperation"></el-switch>
            </el-form-item>
            <operation-config 
              v-if="data.options.showOperation"
              v-model="data.options.operations"
              @on-add-buttonevent="handleButtonEventAdd"
              @on-edit-buttonevent="handleButtonEventEdit"
            ></operation-config>
          </el-collapse-item>
          <el-collapse-item :title="$t('fm.config.widget.pageSetting')" name="paging"
            v-if="data.type == 'list'"
          >
            <el-form-item :label="$t('fm.config.widget.paging')">
              <el-switch v-model="data.options.pagination.paging"></el-switch>
            </el-form-item>
            <el-form-item :label="$t('fm.config.widget.pageSizes')" v-if="data.options.pagination.paging">
              <el-input v-model="data.options.pagination.pageSizes" :placeholder="$t('fm.config.widget.pageSizesPlaceholder')"></el-input>
            </el-form-item>
            <el-form-item :label="$t('fm.config.widget.pagerCount')" v-if="data.options.pagination.paging">
              <el-input-number :step="2" :min="5" :max="21" v-model="data.options.pagination.pagerCount"></el-input-number>
            </el-form-item>
            <el-form-item :label="$t('fm.config.widget.background')" v-if="data.options.pagination.paging">
              <el-switch v-model="data.options.pagination.background"></el-switch>
            </el-form-item>
            <el-form-item :label="$t('fm.config.widget.smallPage')" v-if="data.options.pagination.paging">
              <el-switch v-model="data.options.pagination.small"></el-switch>
            </el-form-item>
            <el-form-item :label="$t('fm.config.widget.dataStruct')" v-if="data.options.pagination.paging">
              <el-input class="fm-options-item" clearable  v-model="data.options.pagination.props.list">
                <template #prepend>{{$t('fm.config.widget.structList')}}</template>
              </el-input>
              <el-input class="fm-options-item" clearable  v-model="data.options.pagination.props.total">
                <template #prepend>{{$t('fm.config.widget.structTotal')}}</template>
              </el-input>
              <el-input class="fm-options-item" clearable v-model="data.options.pagination.props.size">
                <template #prepend>{{$t('fm.config.widget.structSize')}}</template>
              </el-input>
              <el-input class="fm-options-item" clearable v-model="data.options.pagination.props.current">
                <template #prepend>{{$t('fm.config.widget.structCurrent')}}</template>
              </el-input>
            </el-form-item>
          </el-collapse-item>
          <el-collapse-item :title="$t('fm.config.widget.expandSetting')" name="expend">
            <!-- 动态值绑定 -->
            <!-- 
            ...
            "dynamicValueStruct": "" 
            -->
            <el-form-item :label="$t('fm.config.widget.dynamicValue')" v-if="!['grid', 'col', 'report', 'th', 'td', 'tabs', 'collapse', 'inline', 'card', 'divider', 'alert'].includes(data.type)">
              <div><el-switch v-model="data.options.isDynamicValue"></el-switch></div>
              <template v-if="data.options.isDynamicValue">
                <el-radio-group v-model="data.options.dynamicValueType" style="margin-bottom:10px;">
                  <el-radio label="datasource" value="datasource">{{$t('fm.datasource.name')}}</el-radio>
                  <el-radio label="fx" value="fx">{{$t('fm.formula.input.fx')}}</el-radio>
                </el-radio-group>
                <formula-input size="default" v-if="data.options.dynamicValueType == 'fx'" only v-model="data.options.dynamicValueFx" style="margin-bottom: 5px;"></formula-input>
                
                <div v-if="data.options.dynamicValueType == 'datasource'" style="display: flex; align-items: center; margin-bottom: 5px;">
                  <el-select
                    @change="handleDynamicDataSourceChange"
                    v-model="data.options.dynamicValueDataSource">
                    <el-option
                      v-for="item in datasources"
                      :key="item.value"
                      :label="item.label"
                  ...
                  <el-tooltip :content="$t('fm.config.widget.settings')">
                    <el-button link type="primary" style="margin: 5px;" @click="handleDatasourceManage(data.options.dynamicValueDataSource)">
                      <i class="fm-iconfont icon-pm"></i>
                    </el-button>
                  </el-tooltip>
                </div>
                <object-dynamic :key="data.options.dynamicValueDataSource" :operable="false" v-if="data.options.dynamicValueType == 'datasource' && data.options.dynamicValueArgs" v-model="data.options.dynamicValueArgs"></object-dynamic>
                <el-input class="fm-options-item" :placeholder="$t('fm.config.widget.dynamicStructPlaceholder')" clearable  v-model="data.options.dynamicValueStruct" v-if="data.options.dynamicValueType == 'datasource'">
            ...
            <!-- 状态 -->
            <el-form-item :label="$t('fm.config.widget.state')" v-if="data.type != 'td' && data.type != 'th' && data.type != 'col' && data.type != 'dialog'">
              <state-config v-model="data.options.hidden" :label="$t('fm.config.widget.hidden')" v-if="Object.keys(data.options).indexOf('hidden')>=0"></state-config>

              <state-config v-model="data.options.disabled" :label="$t('fm.config.widget.disabled')" v-if="Object.keys(data.options).indexOf('disabled')>=0"></state-config>
            </el-form-item>
            ...
            <el-form-item :label="$t('fm.config.widget.customClass')" v-if="Object.keys(data.options).includes('customClass')">
              ...
              <div style="display: flex; align-items: center;">
                ...
                <el-tooltip :content="$t('fm.config.widget.settings')">
                  <el-button link type="primary" style="margin: 5px;" @click="handleSetClass">
                    <i class="fm-iconfont icon-pm"></i>
                  </el-button>
                </el-tooltip>
              </div>
            ...
            <!-- 属性 -->
            <el-form-item :label="$t('fm.config.widget.attribute')" key="attribute" v-if="data.type != 'td' && data.type != 'th' && data.type != 'col' && data.type != 'button' && data.type != 'link' && data.type != 'alert' && data.type != 'divider'">
            ...
            <!-- 自定义扩展属性 -->
            <template v-if="Object.keys(data.options).includes('extendProps')">
              <el-form-item :label="$t('fm.config.widget.extendPropsConfig')" v-if="Object.keys(data.options).includes('extendProps')">
                <extend-prop-config v-model="data.options.extendProps"></extend-prop-config>
              </el-form-item>
            </template>
              ...
              <el-form-item :label="$t('fm.config.widget.validate')">
                ...
                <div class="validate-block" v-if="Object.keys(data.options).indexOf('validator')>=0">
                  ...
                  <div v-if="data.options.validatorCheck">
                    ...
                    <el-popover placement="right" :width="500" trigger="click" :hide-after="0">
                      <template #reference>
                        <el-button class="message-input">{{$t('fm.config.widget.setting')}}</el-button>
                      </template>
                      <div>
                        <div style="font-size: 14px;font-weight: 500;" class="code-line">(rule, value, callback) => {</div>
                        <code-editor mode="javascript" :key="data.key" v-model="data.options.validator" height="300px"></code-editor>
    ...
    <code-dialog ref="codeDialog" mode="html" :title="$t('fm.config.widget.customTemplates')" help="https://form.making.link/docs/manual/component.html" @on-confirm="handleTemplateConfirm"></code-dialog>
...
import ObjectDynamic from './DataSource/objectDynamic.vue'
import FormulaInput from './FormulaPanel/input.vue'
import ArrayDynamic from './DataSource/arrayDynamic.vue'
import ColumnsConfig from './DataTable/ColumnsConfig.vue'
import OperationConfig from './DataTable/OperationConfig.vue'
import ExtendPropConfig from './ConfigComponent/ExtendProp.vue'
import StateConfig from './ConfigComponent/StateConfig.vue'
...
export default {
  ...
  methods: {
    ...
    setEvent (eventObj) {
      ...
      if (eventObj.type.includes('operate_') && this.data.type == 'list') {
        ...
        if (match) {
          ...
          this.data.options.operations.buttons[index].event = eventObj.key
        }
    ...
    handleButtonEventAdd (index) {
      this.$emit('on-event-add', 'operate_' + index)
    },

    handleButtonEventEdit (index, key) {
      this.$emit('on-event-edit', {
        eventName: 'operate_' + index, 
        functionKey: key
      })
    }
  },
