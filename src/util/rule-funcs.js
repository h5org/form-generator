  ...
  if (rule.action == 'setData') {
    const setField = rule.options.fields.map(item => {
      if (rule.options.valueTypes[item] == 'fx') {
        return `'${item}' : ${rule.options.values[item] || JSON.stringify('')}`
        ...
        return `'${item}' : ${rule.options.values[item] || (typeof rule.options.values[item] === 'number' ? 0 : undefined)}`
      } else {
  ...
  if (rule.action == 'sendRequest') {
    ...
    let argsArray = Object.keys(rule.options.dataSource.args).map(argKey => {
      if (rule.options.valueTypes[argKey] == 'fx') {
        ...
        return `'${argKey}' : ${rule.options.dataSource.args[argKey] || (typeof rule.options.values[item] === 'number' ? 0 : undefined)}`
      } else {
